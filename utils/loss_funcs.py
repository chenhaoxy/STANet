...
import sys
import os
sys.path.append('E:\\CODE\\LearnTrajDep-code for predict\\LearnTrajDep-master\\utils')
...
def sen_loss(outputs, all_seq, dim_used, dct_n):
    ...
    print('---------------', 'all_seq.data.shape:',all_seq.data.shape,'dim_used_len:',dim_used_len,'dim_used:',dim_used,'idct_m:',idct_m,'outputs_t:',outputs_t,'pred_expmap:',pred_expmap,'targ_expmap:',targ_expmap)
...
def mpjpe_error_p3d(outputs, all_seq, dct_n, dim_used):
    ...
    #######old#######
    input_n = 10
    pred_3d = outputs_p3d[:, input_n:, :].contiguous().view(-1, dim_used_len).view(-1, 3)
    targ_3d = all_seq[:, input_n:, dim_used].contiguous().view(-1, dim_used_len).view(-1, 3)
    #######new#######
    # input_n = 10
    # pred_3d = all_seq.clone()
    # pred_3d[:, :, dim_used] = outputs_p3d
    # # pred_3d[:, :, index_to_ignore] = pred_3d[:, :, index_to_equal]
    # pred_3d = pred_3d.contiguous().reshape(n, seq_len, -1, 3)[:, input_n:, :, :].reshape(-1, 3)
    # targ_3d = all_seq.contiguous().reshape(n, seq_len, -1, 3)[:, input_n:, :, :].reshape(-1, 3)
...
def mpjpe_error(outputs, target):
    """
    ...
    :return:
    """
    pred_3d = outputs.view(-1,3)
    targ_3d = target.view(-1,3)
