...
def readCSVasFloat(filename):
    ...
    这个函数是将一个txt文件中的数据先是按照每行读取（可能是以换行符为标记）接着将每行的元素以空格为分界符每个列表为某个关节点的坐标
    在将关节点坐标以，为分界符再划分.划分之后将每个line的元素依次从str转化为float
    lines：是整个文件的数据，每个元素是一行的数据
    line：是一行的数据，处理后每个元素是一个点的坐标
    x：是一个点的坐标，处理后每个元素是一个轴的坐标
    """
    returnArray = []
...
def define_actions(action):
    ...
    #            "sittingdown",  "takingphoto", "waiting", "walkdog",
    #            "walktogether" ]
    actions = ["walking","eating", "smoking", "discussion", "directions",
               "greeting", "phoning", "posing", "purchases", "sitting",
               "sittingdown", "waiting", "walkdog",
               "walktogether" ]
    # actions = ["walking"]
...
def define_actions_cmu(action):
    ...
    #            "washwindow"]
    actions = ["basketball", "basketball_signal", "directing_traffic", "jumping", "soccer", "walking",
               "washwindow"]
    if action in actions:
        return [action]
...
def load_data_cmu_3d(path_to_dataset, actions, input_n, output_n, data_std=0, data_mean=0, is_test=False):
    ...
    joint_to_ignore = np.array([11, 12, 21, 23, 28, 30])
...
def expmap2rotmat_torch(r):
    ...
    # print('r',r.shape,'theta',theta.shape)
...
def load_data(path_to_dataset, subjects, actions, sample_rate, seq_len, input_n=10, data_mean=None, data_std=None):
        ...
        for action_idx in np.arange(len(actions)):
            ...
            if not (subj == 5):
                for subact in [1, 2]:  # subactions
                    ...
                    action_sequence = readCSVasFloat(filename)
                    n,_, d = action_sequence.shape#原代码只给n，d两个元素赋值，因为action_sequence的shape是三维，所以要多加一个横杠，可能是数据预处理的问题
    ...
    # if it is not testing or validation then get the data statistics
...
def load_data_3d(path_to_dataset, subjects, actions, sample_rate, seq_len):
    ...
    :param subjects:
    :param actions:
    :param sample_rate:采样率：可以理解为每几帧取一帧当作输入数据
    :param seq_len:
    :return:
    """
        ...
        for action_idx in np.arange(len(actions)):
            ...
            if not (subj == 5):
                for subact in [1, 2]:  # subactions
                    ...
                    action_sequence = readCSVasFloat(filename)
                    n, _, d = action_sequence.shape
                    even_list = range(0, n, sample_rate)   #输入数据的帧
                    num_frames = len(even_list)  #输入数据的帧数
                    the_sequence = np.array(action_sequence[even_list, :])
                    the_seq = Variable(torch.from_numpy(the_sequence)).float().cuda()  #转为float变量？a
                    # remove global rotation and translation
                    the_seq[:, :, 0:6] = 0
                    # p3 = expmap2xyz_torch(the_seq)
                    # p3 = p3.view(num_frames,96).cpu().data.numpy()
                    p3d = the_seq.view(num_frames,32,3)
                    # p3d = p3d.view(num_frames, 96).cpu().data.numpy()
                    the_sequence = p3d.view(num_frames, -1).cpu().data.numpy()

                    fs = np.arange(0, num_frames - seq_len + 1)   #seq_len是每组训练数据的长度
                    fs_sel = fs
                    for i in np.arange(seq_len - 1):
                        fs_sel = np.vstack((fs_sel, fs + i + 1))
                    fs_sel = fs_sel.transpose()
                    seq_sel = the_sequence[fs_sel, :]  #？？？  shape=(num_frames,seq_len,96)
                    if len(sampled_seq) == 0:
                        sampled_seq = seq_sel
                        complete_seq = the_sequence
                    else:
                ...
                the_sequence1 = action_sequence[even_list, :]
                the_seq1 = Variable(torch.from_numpy(the_sequence1)).float().cuda()
                # the_seq1[:, 0:6] = 0  # 前两组数据置为0，后面也会丢弃
                # p3d1 = expmap2xyz_torch(the_seq1)  # 关节角度数据转化为关节点的坐标
                p3d1 = the_seq1.view(num_frames1,32,3)
                ...
                # r = action_sequence[even_list, :]
                # r = np.squeeze(r,1)
                # t = np.array(r)
                the_sequence2 = np.array(action_sequence[even_list, :]) #问题！！
                the_seq2 = Variable(torch.from_numpy(the_sequence2)).float().cuda()
                # the_seq2[:, 0:6] = 0
                # p3d2 = expmap2xyz_torch(the_seq2)
                p3d2 = the_seq2.view(num_frames2,32,3)
    ...
    # truck_use = [25, 26, 27]
    # joint_to_ignore = np.array([i for i in range(32) if i not in truck_use])
    joint_to_ignore = np.array([5, 10, 11, 12, 16, 20, 23, 24, 28, 31])  # [5, 10, 11, 12, 16, 20, 23, 24, 28, 31] np.array([4, 5, 9, 10, 11, 16, 20, 21, 22, 23, 24, 28, 29, 30, 31])
    dimensions_to_ignore = np.concatenate((joint_to_ignore * 3, joint_to_ignore * 3 + 1, joint_to_ignore * 3 + 2))
    dimensions_to_use = np.setdiff1d(np.arange(complete_seq.shape[1]), dimensions_to_ignore)

    return sampled_seq, dimensions_to_ignore, dimensions_to_use   #sampled_seq是累计读取的所有的数据（例如训练数据或者测试数据）剩余的变量就是忽略的关键点和使用的关键点
#    shape：[-1, seq_len, 96]
